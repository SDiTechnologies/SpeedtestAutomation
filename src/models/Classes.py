...
class OoklaResponse:
    ...
    def from_dict(obj: Any) -> 'OoklaResponse':
        ...
        _packet_loss = float(obj.get("packetLoss")) if obj.get("packetLoss") is not None else "NULL"
        ...
        return OoklaResponse(_type, _local_timestamp, _timestamp, _ping, _download, _upload, _packet_loss, _isp, _interface, _server, _result)
    ...
    def check_notify(self, down_min=25, up_min=3):
        ...
        try:
            ...
            self._logger.log(f'check_notify() results: {result}, {down_mbps} Mbps(download) / {up_mbps} Mbps(upload)')
            return result
